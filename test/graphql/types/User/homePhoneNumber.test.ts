...
describe("homePhoneNumberResolver", () => {
	...
	const createSetup = (
		...
		targetPhone: string | null | undefined = DEFAULT_PHONE,
		isSelfAccess: boolean = false,
	) => {
		const currentUserId = uuid();
		const targetUserId = isSelfAccess ? currentUserId : uuid();
	...
	it("throws unauthenticated error when client is not authenticated", async () => {
		const { context, mocks, parent } = createSetup(false);
